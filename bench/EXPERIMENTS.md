...
| 35 | Equal-wall grids, `69dcc23` vs `89d92a5` | Money 352 cells (1 thread) and ordinary 464 cells (5 threads per side, `PB_GRAN=2` above size 100) via `bench/equal_wall_runner.py`; unit `commiv-ew-hash`, rows in `~/projects/equal-wall-hash/` on auregat | `89d92a5` | `69dcc23` | pending | pending | PENDING | This is the quality verdict for the trajectory change in row 33; losses are expected here because the new binary is a different sample path, not a superset |
