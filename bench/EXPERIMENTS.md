...
| 28 | CVRP transposed rows in SISR recreate | The two column-wise loads per neighbour (`d(p, c)`, `d(m, c)`) become row reads of a transposed copy | `b9a961b`, reverted `a8946fc` | `9be2099` | idle box, interleaved: old 2030 / 2041, new 2062 / 2051 ms | +1.6% / +0.5% | DEAD | Rows p and m are fetched anyway for the two route arcs, same failure mode as experiment 20 |
| 29 | Insertion sort for the recreate demand order | The per-recreate `pdq` over the removed set (7% self) has a total order (demand desc, index asc), so any correct sort is exact; sets are at most ~40 customers | `28cbe08` | `9be2099` | idle box, interleaved: old 2046 / 2047, new 1869 / 1881 ms | -8.7% / -8.1% | WIN | Signature `74102 / 43` identical on all 20 runs; `pdq` kept for sets above 48 |
| 30 | PDPTW install with evaluator labels | `install` re-walks the route for `arcSum` and `routeDuration` although the chosen candidate's distance and duration are known | `1f68d17`, reverted `d8d3b8c` | `f7bca03` | idle box, interleaved: ordinary old 1122 / 1129, new 1132 / 1123 ms; money old 1670 / 1652, new 1639 / 1620 ms | ordinary +0.9% / -0.5%; money -1.9% / -1.9% | DEAD | Signatures identical in both modes; below the 3% margin |
| 31 | Exact pruning of the fast gap scan (probe, not landed) | Any gap skipped before its blink draw shifts the RNG stream; a q-window dominance break (`depart + min d(., q) > due[q]`) shows it | not committed | `f7bca03` | ordinary: signature moves to 25 veh / 47248.490 / 154057.648 and the run takes 1800 / 1798 / 1815 ms (row minimum per call) vs 1122 | n/a | IMPOSSIBLE EXACTLY | The visited (a, b) set is defined by the existing feasibility breaks; every visited pair consumes one draw, so only per-pair work can shrink, not the visit set |
| 32 | Branch tip gates vs `4312d28` | Final state of every kept lever, idle box | `21a0a5f` | `4312d28` | ordinary PDPTW old 1204 / 1206, new 1113 / 1149 ms; money old 1853 / 1855, new 1640 / 1653 ms; VRPTW old 2438 / 2437, new 2122 / 2109 ms; CVRP old 2477 / 2463, new 1885 / 1887 ms | -7.6% / -4.7%; -11.5% / -10.9%; -13.0% / -13.5%; -23.9% / -23.4% | WIN | All signatures identical; 52-case corpus byte-identical to `887e520`; `zig build test` rc 0 in Debug and ReleaseFast |
| 33 | Hash blink per gap site | `blink.zig hash(seed, generation, route, p, a, b)` replaces the shared-PRNG draw in all three PDPTW pair evaluators, so a blink no longer depends on how many gaps were visited before it | `a7255ba` | `69dcc23` | ordinary: old 1151 / 1132 ms, new 1229 / 1228 ms; money: old 1651 / 1675 ms, new 1740 / 1744 ms (`taskset -c 2`, medians of 5, interleaved) | +6.8% / +8.5%; +5.4% / +4.1% | TRAJECTORY CHANGE | Signature moves by design: ordinary 23 veh / 45943.646 / 142318.993 / 86375.347, money 22 veh / 61113.307 / 119996.641 / 48883.334. Same distribution over trajectories (i.i.d. blink per position, Christiaens & Vansteenwegen 2020), different sample path, so quality is judged by the equal-wall grids in row 35, not by this harness. The wall increase is not separable from the new trajectory (23 routes are longer than 25) |
| 34 | Dominance breaks, blink last | Once the middle walk's departure plus `min_j d(j, q)` passes `due[q]` no later dropoff gap is feasible (Campbell & Savelsbergh 2004 style bound, `dmin_in` computed once per solve from the transposed rows); same bound on the pickup prefix; blink evaluated only for gaps that passed every feasibility test | `89d92a5` | `a7255ba` | ordinary: 1229 / 1228 -> 878 / 875 ms; money: 1740 / 1744 -> 1199 / 1206 ms; other 1000-series at 20k iters (B -> C): lc1 238 -> 219, lr1 431 -> 352, lrc1 401 -> 338, lc2 336 -> 285, lrc2 1058 -> 764 ms | -28.6% / -28.7%; -31.1% / -30.9%; -8% / -18% / -16% / -15% / -28% | WIN | Signature identical to row 33 on every run and instance (a pruned gap is infeasible, so its blink could never have inserted), 52-case REST corpus byte-identical to `69dcc23`, `zig build test` rc 0 Debug (2:27) and ReleaseFast (1:47). Versus `69dcc23`: ordinary -23.7% / -22.7%, money -27.4% / -28.0%. Reproduce: `taskset -c 2 python3 bench/run.py --binary <bin> --runs 5 --iters 20000 --seed 1 [--time-pen 1 --veh-pen 280000] [--instance lc1_10_1]` |
| 35 | Equal-wall grids, `69dcc23` vs `89d92a5` | Money 352 cells (1 thread) and ordinary 464 cells (5 threads per side, `PB_GRAN=2` above size 100) via `bench/equal_wall_runner.py`; unit `commiv-ew-hash`, rows in `bench/equal-wall-hash-{money,pdptw}.jsonl` | `89d92a5` | `69dcc23` | money: new W/T/L 157/73/122, $10,966,778.03 -> $10,967,825.03; ordinary: 136/214/114, 9639 -> 9640 veh, 5,081,140.9 -> 5,067,328.2 dist; zero incomplete cells either side | money +0.010% (sign p 0.042 in new's favour by count), ordinary -0.272% distance and +1 vehicle (sign p 0.18) | WASH, KEPT | Quality is indistinguishable from the old trajectory in both modes, which is the pass condition for a trajectory change (row 33); the fixed-work speedup of row 34 stands. Size 1000 money is 26/2/30, +0.136% (p 0.69, one seed). Scorer: `python3 bench/equal_wall_score.py money bench/equal-wall-hash-money.jsonl` |
