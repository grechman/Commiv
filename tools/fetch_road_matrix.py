...
"""
import argparse, json, math, random, time, urllib.request, urllib.error, sys
...
def fetch_matrix(server, coords, max_table=100, pause=1.5, timeout=60):
...
def main():
    ...
    dur = fetch_matrix(args.server, coords, max_table=args.max_table, timeout=args.timeout)
    n = len(dur)
