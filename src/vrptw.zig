...
const SisrTw = struct {
    ...
    fn recreate(s: *SisrTw, params: VrptwSisrParams, rng: std.Random) !void {
        ...
        for (s.removed.items) |c| {
            ...
            if (best_ri == NO_ROUTE and !may_open) {
                if (params.eject) _ = try s.squeezeInsert(c);
    ...
    /// position among its kNN candidate routes, then eject the cheapest
    /// single resident customer to restore feasibility (see ejectCandidate).
    /// The ejected customer goes to the request bank via a bank=false
    /// removal (recreate is iterating
    ...
    fn squeezeInsert(s: *SisrTw, c: usize) !bool {
        s.generation += 1;
        var best_ri: usize = NO_ROUTE;
        var best_g: usize = 0;
