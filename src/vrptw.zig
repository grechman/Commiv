...
// =============================================================================
// SISR for VRPTW (Christiaens & Vanden Berghe 2020, the flagship CVRP engine,
// with time windows wired into recreate). One solution, `iters` cheap ruin+
// recreate steps under threshold accepting — replaces the ILS's thousand full
// local-search convergences with millions of O(removed) moves.
//
...
pub const VrptwSisrParams = struct {
    ...
    l_max: usize = 10, // maximum removed-string length
    cbar: f64 = 10.0, // average customers removed per ruin
    blink: f64 = 0.01, // recreate: probability of skipping a candidate gap
    ...
    // existing single-customer ejectCandidate; rungs 2 and 3 brute-force
    // small subsets of resident customers. Measured on the PDPTW sibling
    // engine: on packed instances 63% of squeezes find no single resident
    // whose removal restores feasibility — ejecting a subset unlocks those.
    // Clamped to [1, 3]; 0 is treated as 1. Only meaningful when eject = true.
    eject_k: u8 = 1,
...
const SisrTwRoute = struct {
    ...
    fn deinit(r: *SisrTwRoute, allocator: std.mem.Allocator) void {
        ...
        r.pre.deinit(allocator);
        r.suf.deinit(allocator);
    }
};
...
const SisrTw = struct {
    ...
    fn arcSum(inst: VrptwInstance, items: []const usize) u64 {
    ...
    /// Append a route slot (grows the per-route mark arrays alongside).
    fn addSlot(s: *SisrTw) !usize {
    ...
    /// Install `nodes` as route `ri`'s content and refresh bookkeeping.
    fn installRoute(s: *SisrTw, ri: usize, nodes: []const usize) !void {
    ...
    /// insertion can shorten the route.
    fn gapDelta(s: *SisrTw, ri: usize, g: usize, c: usize) ?i64 {
        const r = &s.routes.items[ri];
        const it = r.items.items;
    ...
    fn ruin(s: *SisrTw, params: VrptwSisrParams, rng: std.Random, it: usize) !void {
        ...
        while (strings < ks and t <= s.gk) : (t += 1) {
            const c = if (t == 0) seed_c else s.gran[(seed_c - 1) * s.gk + (t - 1)];
            if (c == 0) continue; // kNN padding
            const ri = s.loc_route[c];
            if (ri == NO_ROUTE) continue; // already removed this iteration
            if (s.ruin_mark[ri] == s.generation) continue; // one string per route
            s.ruin_mark[ri] = s.generation;
    ...
    /// Feasibility of dropping `drop` (a small set of resident customers,
    /// never including the just-squeezed-in `c`) from route `ri`'s CURRENT
    /// content: an O(L) forward Tws-merge fold over the route skipping the
    /// dropped customers (mirrors routeTws) plus a running load sum against
    /// capacity. Used by the eject_k >= 2 ladder, where the dropped set is
    /// non-contiguous so the pre/suf O(1) trick ejectCandidate uses does not
    /// apply.
    fn keepFeasible(s: *SisrTw, ri: usize, drop: []const usize) bool {
        const it = s.routes.items[ri].items.items;
        var acc = Tws.depotNode(s.inst);
        var prev: usize = 0;
        var load: u64 = 0;
        scan: for (it) |x| {
            for (drop) |d| if (x == d) continue :scan;
            acc = Tws.merge(acc, @intCast(s.inst.d(prev, x)), Tws.client(s.inst, x));
            if (acc.tw != 0) return false;
            load += s.inst.demand[x];
            if (load > @as(u64, s.inst.capacity)) return false;
            prev = x;
        }
    ...
    /// every resident customer of route `ri` other than `skip`, ordered by
    /// (eject_pen asc, id asc) and truncated to the 16 lowest-key entries —
    /// the ladder brute-forces subsets over this pool, so it must stay small.
    /// Returns the count written into `buf`.
    fn ejectPool(s: *SisrTw, ri: usize, skip: usize, buf: *[16]usize) usize {
        const it = s.routes.items[ri].items.items;
        var n: usize = 0;
        for (it) |x| {
            if (x == skip) continue;
            if (n < buf.len) {
                var pos = n;
                while (pos > 0 and s.ejectPoolLess(x, buf[pos - 1])) : (pos -= 1) buf[pos] = buf[pos - 1];
                buf[pos] = x;
                n += 1;
            } else if (s.ejectPoolLess(x, buf[n - 1])) {
                var pos = n - 1;
                while (pos > 0 and s.ejectPoolLess(x, buf[pos - 1])) : (pos -= 1) buf[pos] = buf[pos - 1];
                buf[pos] = x;
            }
    ...
    fn ejectPoolLess(s: *const SisrTw, a: usize, b: usize) bool {
    ...
    /// ejectPool(ri, skip) in lexicographic index order, feasibility-check
    /// each via keepFeasible, and keep the FEASIBLE subset minimizing (sum
    /// eject_pen over the subset, then lexicographically smallest id tuple).
    /// Deterministic, no RNG. Returns the chosen subset or null if no
    /// `size`-subset of the pool restores feasibility.
    fn ejectSubsetK(s: *SisrTw, ri: usize, skip: usize, size: usize) ?EjectSubset {
        var pool: [16]usize = undefined;
        const n = s.ejectPool(ri, skip, &pool);
        ...
        while (true) {
            ...
            if (s.keepFeasible(ri, cand[0..size])) {
                var pen: u64 = 0;
    ...
    /// position among its kNN candidate routes, then eject residents to
    /// restore feasibility — rung 1 (unchanged) tries a single resident
    /// customer; if that fails and eject_k allows it, rungs 2-3 brute-force
    /// small subsets (see ejectSubsetK). The ejected customers go to the
    /// request bank via bank=false removals (recreate is iterating
    /// s.removed — see removeStringImpl). Returns true on success; on total
    /// failure `c` is removed again (also bank=false) and stays banked.
    fn squeezeInsert(s: *SisrTw, c: usize, params: VrptwSisrParams) !bool {
        ...
        if (try s.ejectCandidate(best_ri, c)) |ec| {
            try s.removeStringImpl(best_ri, s.loc_pos[ec], 1, false);
            s.eject_pen[c] +|= 1;
            return true;
        }

        // Rungs 2-3: k-eject when a single resident customer can't restore
        // feasibility (see VrptwSisrParams.eject_k).
        const eject_k: usize = @min(@max(params.eject_k, 1), 3);
        var size: usize = 2;
        while (size <= eject_k) : (size += 1) {
            const sub = s.ejectSubsetK(best_ri, c, size) orelse continue;
            // loc_pos shifts after each removal — re-read it per member.
            for (sub.buf[0..sub.len]) |e| {
                try s.removeStringImpl(best_ri, s.loc_pos[e], 1, false);
            }
            s.eject_pen[c] +|= 1;
            return true;
        }
    ...
    fn toResult(s: *SisrTw, allocator: std.mem.Allocator) !VrptwResult {
        ...
        for (s.routes.items) |r| {
            if (r.items.items.len == 0) continue;
            routes[filled] = try allocator.dupe(usize, r.items.items);
            filled += 1;
            total += r.dist;
        }
        return .{ .allocator = allocator, .routes = routes, .total_cost = total, .vehicles = s.nonempty };
    }
};
...
pub fn solveVrptwSisrFrom(allocator: std.mem.Allocator, inst: VrptwInstance, options: solver.SolveOptions, params: VrptwSisrParams, warm: []const []const usize) !VrptwResult {
    ...
    if (inst.demand.len != n + 1 or inst.matrix.len != (std.math.mul(usize, n + 1, n + 1) catch return error.InvalidInstance)) return error.InvalidInstance;
    if (inst.ready.len != n + 1 or inst.due.len != n + 1 or inst.service.len != n + 1) return error.InvalidInstance;
    if (inst.demand[0] != 0 or inst.ready[0] != 0 or inst.service[0] != 0) return error.InvalidInstance;
    if (n == 0) return error.InvalidInstance;
    for (1..n + 1) |c| {
        if (inst.demand[c] > inst.capacity) return error.Infeasible;
        const start = @max(inst.d(0, c), inst.ready[c]);
    ...
    if (warm.len == 0) {
        ...
        var i = n;
        while (i > 0) {
            try bounds.append(allocator, .{ sp.pred[i], i });
            i = sp.pred[i];
        }
        var bi = bounds.items.len;
        while (bi > 0) {
            bi -= 1;
...
// --- Best-of-K parallel SISR-TW (same shape as cvrp_sisr's parallel driver) ---
const SisrTwSlot = struct {
    inst: VrptwInstance,
...
fn sisrTwWorker(slot: *SisrTwSlot) void {
    ...
    slot.dist = res.total_cost;
    slot.ok = true;
}

/// Run `threads` independent SISR-TW chains and return the best by the full
/// objective (distance + veh_penalty per route). threads <= 1 is the serial
/// path. Same `threads == 0` machine-dependence caveat as the CVRP variant.
pub fn solveVrptwSisrParallel(allocator: std.mem.Allocator, inst: VrptwInstance, options: solver.SolveOptions, params: VrptwSisrParams, threads: usize) !VrptwResult {
    const cpus = std.Thread.getCpuCount() catch 1;
    const k = if (threads == 0) @max(@as(usize, 1), cpus -| 1) else threads;
    if (k <= 1 or inst.n <= 2) return solveVrptwSisr(allocator, inst, options, params);
    const n = inst.n;

    const slots = try allocator.alloc(SisrTwSlot, k);
    ...
    var spawned: usize = 0;
    for (0..k) |i| {
        ths[i] = std.Thread.spawn(.{}, sisrTwWorker, .{&slots[i]}) catch break;
        spawned += 1;
    }
    for (spawned..k) |i| sisrTwWorker(&slots[i]);
    for (0..spawned) |i| ths[i].join();

    var best: ?usize = null;
    var best_obj: u64 = std.math.maxInt(u64);
    for (slots, 0..) |s, i| {
        if (!s.ok) continue;
        const obj = s.dist + params.veh_penalty * s.nroutes;
        if (best == null or obj < best_obj) {
            best = i;
            best_obj = obj;
        }
    }
...
test "VRPTW SISR parallel: best-of-K is feasible and no worse than one chain" {
    ...
    for (1..dim) |i| {
        ...
        ready[i] = rng.intRangeAtMost(u32, 0, 200);
        due[i] = ready[i] + rng.intRangeAtMost(u32, 100, 500);
        service[i] = rng.intRangeAtMost(u32, 1, 10);
    }
    const inst = VrptwInstance{ .n = n, .matrix = matrix, .demand = demand, .capacity = 10, .ready = ready, .due = due, .service = service };
    var serial = try solveVrptwSisr(allocator, inst, .{ .seed = 3 }, .{ .iters = 4000 });
    defer serial.deinit();
    var par = try solveVrptwSisrParallel(allocator, inst, .{ .seed = 3 }, .{ .iters = 4000 }, 3);
    defer par.deinit();
    try std.testing.expect(validate(inst, par.routes) != null);
    try std.testing.expect(par.total_cost <= serial.total_cost); // chain seed+0 == serial
}

test "VRPTW SISR: feasible on random instances, cost matches the validator" {
    const allocator = std.testing.allocator;
    var prng = std.Random.DefaultPrng.init(0xC0FFEE);
    const rng = prng.random();
    for (0..8) |trial| {
        const n = 4 + rng.uintLessThan(usize, 12);
        const dim = n + 1;
        const matrix = try allocator.alloc(u32, dim * dim);
        defer allocator.free(matrix);
        for (0..dim) |i| {
        ...
        demand[0] = 0;
        ready[0] = 0;
        due[0] = 100_000; // generous horizon: keeps every random instance feasible
        service[0] = 0;
        for (1..dim) |i| {
            demand[i] = rng.intRangeAtMost(u32, 1, 5);
            ready[i] = rng.intRangeAtMost(u32, 0, 300);
        ...
        const inst = VrptwInstance{ .n = n, .matrix = matrix, .demand = demand, .capacity = 12, .ready = ready, .due = due, .service = service };
        var res = try solveVrptwSisr(allocator, inst, .{ .seed = trial + 1 }, .{ .iters = 3000 });
        defer res.deinit();
        const checked = validate(inst, res.routes) orelse return error.TestInfeasibleResult;
        try std.testing.expectEqual(res.total_cost, checked);
...
test "VRPTW SISR surfaces infeasibility as a clean error" {
    ...
    const demand = [_]u32{ 0, 1, 1 };
    const service = [_]u32{ 0, 5, 5 };
    // customer 1's window closes before the depot can reach it
    const ready = [_]u32{ 0, 0, 0 };
    const due = [_]u32{ 1000, 2, 500 };
    const inst = VrptwInstance{ .n = 2, .matrix = &m, .demand = &demand, .capacity = 5, .ready = &ready, .due = &due, .service = &service };
    try std.testing.expectError(error.Infeasible, solveVrptwSisr(allocator, inst, .{ .seed = 1 }, .{ .iters = 100 }));
    const bad_depot = [_]u32{ 2, 1, 1 };
    const due_ok = [_]u32{ 1000, 500, 500 };
    const inst2 = VrptwInstance{ .n = 2, .matrix = &m, .demand = &bad_depot, .capacity = 5, .ready = &ready, .due = &due_ok, .service = &service };
    try std.testing.expectError(error.InvalidInstance, solveVrptwSisr(allocator, inst2, .{ .seed = 1 }, .{ .iters = 100 }));
}
...
test "VRPTW SISR eject_k: constructed 2-eject unlocks what single eject cannot" {
    // Hand-built route: residents A(1) and B(2), demand 5 each, are already
    // in the route; customer C(3), demand 6, is squeezed in ahead of them
    // (as squeezeInsert leaves the route after insertAt, before ejection).
    // Total load 5+5+6=16 against capacity 10. Removing either resident
    // alone still leaves 5+6=11 > 10; removing BOTH leaves just C's own
    // 6 <= 10. Wide TWs so only load binds, per the squeeze motivation.
    const allocator = std.testing.allocator;
    const dim = 4;
    var matrix: [16]u32 = undefined;
    for (0..dim) |a| for (0..dim) |b| {
        matrix[a * dim + b] = if (a == b) 0 else 10;
    };
    const demand = [_]u32{ 0, 5, 5, 6 };
    const ready = [_]u32{ 0, 0, 0, 0 };
    const due = [_]u32{ 100_000, 100_000, 100_000, 100_000 };
    const service = [_]u32{ 0, 0, 0, 0 };
    const inst = VrptwInstance{
        .n = 3,
        .matrix = matrix[0..],
        .demand = demand[0..],
    ...
    const gran = try buildNeighbors(allocator, inst, 2);
    defer allocator.free(gran);
    var s = try SisrTw.init(allocator, inst, 10_000_000, gran, 2);
    defer s.deinit();
    const ri = try s.addSlot();
    try s.installRoute(ri, &[_]usize{ 1, 2, 3 });

    // rung 1: no single resident restores feasibility
    try std.testing.expectEqual(@as(?usize, null), try s.ejectCandidate(ri, 3));
    // rung 2: ejecting both 1 and 2 does
    const sub = s.ejectSubsetK(ri, 3, 2) orelse return error.TestExpectedEject;
    ...
    try std.testing.expectEqualSlices(usize, &[_]usize{ 1, 2 }, &got);
}

test "VRPTW SISR eject_k: default 1 leaves engine bit-identical" {
    const allocator = std.testing.allocator;
    var seed: u64 = 1;
    while (seed <= 3) : (seed += 1) {
        var prng = std.Random.DefaultPrng.init(0xBEEF0000 ^ seed);
        const rng = prng.random();
        const n = 40;
        const dim = n + 1;
        const matrix = try allocator.alloc(u32, dim * dim);
        ...
        const ra = solveVrptwSisr(allocator, inst, .{ .seed = seed }, .{
            .iters = 6000,
            .veh_penalty = 10_000_000,
            .max_vehicles = target,
            .eject = true,
        });
        const rb = solveVrptwSisr(allocator, inst, .{ .seed = seed }, .{
            .iters = 6000,
            .veh_penalty = 10_000_000,
            .max_vehicles = target,
            .eject = true,
...
test "VRPTW SISR eject_k: deep ejection churn (k=3) stays valid and cost-honest" {
    // Same harness as the "capped: request bank" test above, but with the
    // ladder's deepest rung enabled: asserts no crash and any returned
    // solution passes the independent oracle at the exact reported cost
    // under maximum 3-eject churn.
    const allocator = std.testing.allocator;
    var seed: u64 = 1;
    while (seed <= 3) : (seed += 1) {
        var prng = std.Random.DefaultPrng.init(0xCA5CADE ^ seed);
        const rng = prng.random();
        const n = 40;
        const dim = n + 1;
        const matrix = try allocator.alloc(u32, dim * dim);
