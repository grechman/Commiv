...
/// Targeted variant of segmentExchangeKick: each of the three cut positions is
/// chosen by a tournament of `tk` random draws, keeping the one with the highest
/// edge REGRET = d(tour[p-1], tour[p]) - min(nearestCand(tour[p-1]),
/// nearestCand(tour[p])). Regret beats raw length at large n: a legitimately
/// long optimal edge has low regret (both endpoints' nearest candidate is also
/// far), while a wrong edge — long despite a much closer candidate sitting
/// unused — has high regret. So this aims the perturbation + follow-up LK repair
/// at genuinely suspect edges. The tournament keeps randomness for diversity.
/// Same rotate + touched contract as segmentExchangeKick. tk <= 1 ~ uniform.
pub fn segmentExchangeKickTargeted(
    dist_oracle: *DistanceOracle,
    candidates: *const Candidates,
    tour: []usize,
    random: *std.Random,
    ...
    const n = tour.len;
    std.debug.assert(n >= 8);
    const i = highestRegretPos(dist_oracle, candidates, tour, random, 1, n - 2, tk);
    const j = highestRegretPos(dist_oracle, candidates, tour, random, i + 1, n - 1, tk);
    const k = highestRegretPos(dist_oracle, candidates, tour, random, j + 1, n, tk);
    touched.* = .{ tour[i - 1], tour[i], tour[j - 1], tour[j], tour[k - 1], tour[k] };
    std.mem.rotate(usize, tour[i..k], j - i);
}

/// Nearest candidate distance for `node` (min over its candidate row; alpha
/// rows are alpha-sorted, not distance-sorted, so scan the whole short row).
fn nearestCandDist(candidates: *const Candidates, node: usize) u64 {
    var m: u64 = std.math.maxInt(u64);
    for (0..candidates.width) |k| {
        const d = candidates.candDist(node, k);
        if (d < m) m = d;
    }
    return m;
}

/// Edge regret at position p: d(a,b) - min(nearestCand(a), nearestCand(b)),
/// saturating at 0. High when the tour edge is long despite a close unused
/// candidate at an endpoint.
fn edgeRegret(dist_oracle: *DistanceOracle, candidates: *const Candidates, tour: []const usize, p: usize) u64 {
    const a = tour[p - 1];
    const b = tour[p];
    const len = dist_oracle.distance(a, b);
    const floor = @min(nearestCandDist(candidates, a), nearestCandDist(candidates, b));
    return len -| floor;
}

/// Tournament: return the position in [lo, hi) with the highest edge regret
/// among `tk` uniform draws (at least one draw).
fn highestRegretPos(dist_oracle: *DistanceOracle, candidates: *const Candidates, tour: []const usize, random: *std.Random, lo: usize, hi: usize, tk: usize) usize {
    var best = random.intRangeLessThan(usize, lo, hi);
    var best_r = edgeRegret(dist_oracle, candidates, tour, best);
    var t: usize = 1;
    while (t < tk) : (t += 1) {
        const cand = random.intRangeLessThan(usize, lo, hi);
        const r = edgeRegret(dist_oracle, candidates, tour, cand);
        if (r > best_r) {
            best = cand;
            best_r = r;
        }
