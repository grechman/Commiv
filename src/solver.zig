...
pub const buildCandidatesAuto = candidates_mod.buildCandidatesAuto;
...
pub const SolveOptions = struct {
    ...
    // Opt-in candidate injection (default off → identical behaviour). When set,
    // solve() uses these candidate lists verbatim instead of building its own.
    // The caller owns the struct (solve does not deinit it). Used by the ATSP
    ...
    pub const Search = struct {
        ...
        // large n (the "richer LK moves" lever).
...
pub fn solve(
    ...
    if (options.injected_candidates) |inj| {
        candidates = inj.*; // shallow view: shares arrays, caller owns them
    } else {
        candidates = try candidates_mod.buildCandidatesAuto(
            allocator,
            &oracle,
            width,
    ...
    while (trial < max_trials and trial - last_improvement_trial < trials) : (trial += 1) {
        ...
        var search = LocalSearch{
            ...
            .nonseq_max_dimension = options.search.nonseq_max_dimension,
        };
        search.rebuildState();
        if (kick_trial) {
...
/// Recombine a set of independent, already-solved tours via EAX merge + re-opt,
/// returning the best tour found (>= the best input). Tests the
/// independent-multistart-recombination lever (2026-06-17 finding): the
/// diversity across independent restarts is real and large (~73% of a single
/// run's missed optimal edges appear in some other run), but a single
/// incumbent's elite pool — drawn from one trajectory — cannot capture it.
/// This pass loads the K inputs as the population and EAX-merges every pair to a
/// fixpoint, re-feeding improved products into the pool so gains compound.
///
pub const SubproblemRefineOptions = struct {
    // Defaults from the 2026-06-19 sweep: gap falls with BOTH window size (knee
    // ~1000) and sub-solve quality (sub_trials — 100 badly under-solves the
    // sub-TSPs and leaves gains on the table; 300-800 keeps dropping). These are
    // a strong-but-not-maximal point; raise window toward 1000 and sub_trials
    // toward 800 for the lowest gap at higher cost, or use a multi-scale schedule.
    window: usize = 800,
    step: usize = 200,
    max_passes: usize = 8,
    sub_trials: usize = 300,
};

/// Windowed subproblem re-optimization of `tour` (a valid n-permutation), in
/// place. Slides overlapping windows of `window` consecutive tour positions,
/// re-solving each as a fresh open-path sub-TSP between its two EXTERNAL anchor
/// nodes (a dummy node forces those anchors to be the path endpoints), splicing
/// back only on strict improvement (monotone — never lengthens the tour). The
/// fresh per-region solve is a far larger neighborhood than LK local moves and
/// rebuilds the region interior AND its two boundary stitches, recovering the
/// global-structural large-n gap a local optimum hides. Measured on the
/// converged tour: rl11849 0.85% -> ~0.34%, d18512 1.25% -> ~0.55%, generalizing
/// where the strong-LK polish found nothing. Builds its own oracle from `p`;
/// returns the refined tour length.
pub fn refineBySubproblem(
    allocator: std.mem.Allocator,
    p: *const problem.Problem,
    base_options: SolveOptions,
    refine: SubproblemRefineOptions,
    tour: []usize,
) !u64 {
    const n = p.dimension;
    var oracle = try DistanceOracle.init(allocator, p, base_options.budget.max_distance_cache_bytes);
    defer oracle.deinit();
    const W = refine.window;
    const step = @max(refine.step, 1);
    if (n < 16 or W < 3 or W + 2 >= n) return oracle.tourLengthUnchecked(tour);

    const m_max = W + 3; // anchorL + W interior + anchorR + dummy
    const orig = try allocator.alloc(usize, W + 2);
    defer allocator.free(orig);
    const mat = try allocator.alloc(u32, m_max * m_max);
    defer allocator.free(mat);
    const succ = try allocator.alloc(usize, m_max);
    defer allocator.free(succ);
    const order = try allocator.alloc(usize, m_max);
    defer allocator.free(order);
    const seq = try allocator.alloc(usize, m_max);
    defer allocator.free(seq);

    var zero_streak: usize = 0;
    var pass: usize = 0;
    while (pass < refine.max_passes) : (pass += 1) {
        // Shift window phase each pass so stitches at a window edge last pass
        // become interior (re-solvable) this pass.
        const phase = 1 + (pass * step) / @max(refine.max_passes, 1) % step;
        var lo = phase;
        var pass_splices: usize = 0;
        while (lo + W < n) : (lo += step) {
            if (try refineWindow(allocator, &oracle, base_options, tour, lo, W, refine.sub_trials, orig, mat, succ, order, seq)) pass_splices += 1;
        }
        if (pass_splices == 0) {
            zero_streak += 1;
    ...
    return oracle.tourLengthUnchecked(tour);
}

/// Run `refineBySubproblem` over a schedule of (typically increasing) window
/// sizes on the same tour in place. Measured far below a single window: small
/// windows fix fine structure, larger windows fix inter-region boundaries the
/// small ones can't (the fixed anchors sit further apart, freeing the re-solve).
/// rl11849 single W=500 0.34% -> schedule {300,600,1000,1500,2000} ~0.24%.
/// Returns the final tour length.
pub fn refineBySubproblemSchedule(
    allocator: std.mem.Allocator,
    p: *const problem.Problem,
    base_options: SolveOptions,
    schedule: []const SubproblemRefineOptions,
    tour: []usize,
) !u64 {
    var len: u64 = 0;
    for (schedule) |ro| len = try refineBySubproblem(allocator, p, base_options, ro, tour);
    return len;
}
...
fn refineWindow(
    ...
    const hi = lo + W - 1;
    const count = W + 2; // anchorL + interior + anchorR
    orig[0] = tour[lo - 1];
    for (0..W) |k| orig[k + 1] = tour[lo + k];
    orig[W + 1] = tour[hi + 1];

    var cur_path: u64 = 0;
    for (0..count - 1) |k| cur_path += oracle.distance(orig[k], orig[k + 1]);

    const m = count + 1;
    const BIG: u32 = 1_000_000_000;
    for (0..count) |a| {
        for (0..count) |b| mat[a * m + b] = if (a == b) 0 else oracle.distance(orig[a], orig[b]);
    }
    const dummy = count;
    for (0..count) |k| {
        const d: u32 = if (k == 0 or k == count - 1) 0 else BIG;
        mat[dummy * m + k] = d;
        mat[k * m + dummy] = d;
    }
    mat[dummy * m + dummy] = 0;

    var sub = try problem.Problem.initFullMatrix(allocator, "seg", m, mat[0 .. m * m]);
    defer sub.deinit();
    var res = try solve(allocator, &sub, .{
        .seed = base_options.seed,
        .budget = .{ .trials = sub_trials, .trial_extension_factor = 2, .max_passes = 64 },
        .candidates = .{ .candidate_count = @min(@as(usize, 10), m - 1), .candidate_mode = .alpha_nearness, .sparse_min_dimension = 0 },
        .search = .{ .enable_lk = true, .lk_max_depth = 5, .lk_backtrack_limit = 80_000 },
    });
    defer res.deinit();
    if (res.length >= cur_path) return false; // strict improvement only (monotone)

    // res.tour is a closed tour over 0..m-1; dummy must sit between the two
    // anchors (sub 0 and sub count-1). Reconstruct the open path and splice.
    for (res.tour, 0..) |node, idx| succ[node] = res.tour[(idx + 1) % m];
    var len_seq: usize = 0;
    var walk: usize = 0;
    while (true) {
        order[len_seq] = walk;
        ...
        walk = succ[walk];
        if (walk == 0) break;
        if (len_seq > m) return false;
    }
    if (len_seq != m) return false;
    var di: usize = 0;
    while (di < len_seq and order[di] != dummy) : (di += 1) {}
    if (di >= len_seq) return false;
    const left = order[(di + len_seq - 1) % len_seq];
    const right = order[(di + 1) % len_seq];
    if (!((left == 0 and right == count - 1) or (left == count - 1 and right == 0))) return false;

    var sl: usize = 0;
    var idx2: usize = (di + 1) % len_seq;
    while (order[idx2] != dummy) {
        seq[sl] = order[idx2];
        sl += 1;
        idx2 = (idx2 + 1) % len_seq;
    }
    if (sl != count) return false;
    if (seq[0] != 0) {
        var i: usize = 0;
        var j: usize = sl - 1;
        while (i < j) : ({
            i += 1;
            j -= 1;
        }) std.mem.swap(usize, &seq[i], &seq[j]);
    }
    if (seq[0] != 0 or seq[count - 1] != count - 1) return false;
    for (0..count) |k| tour[lo - 1 + k] = orig[seq[k]];
    return true;
}

/// Caller owns `tours`/`lens` (each tour a valid n-permutation, lens its
/// length); the returned SolveResult.tour is owned by `allocator`. Builds its
/// own oracle/candidates (EAX needs them), so it is a standalone phase that does
/// not touch solve()'s tuned trajectory.
pub fn recombineTours(
    allocator: std.mem.Allocator,
    p: *const problem.Problem,
    options: SolveOptions,
    tours: []const []const usize,
    lens: []const u64,
) !SolveResult {
    const n = p.dimension;
    std.debug.assert(tours.len == lens.len);
    if (tours.len == 0) return error.NoTours;
    ...
    var stats = SolveStats{ .candidate_count = width };

    // Seed the population with the inputs; track the best as the incumbent.
    const best_tour = try allocator.alloc(usize, n);
    errdefer allocator.free(best_tour);
    var best_len: u64 = std.math.maxInt(u64);
    for (tours, lens) |t, l| {
        std.debug.assert(t.len == n);
        if (l < best_len) {
            best_len = l;
            @memcpy(best_tour, t);
        }
        elitePoolOffer(&elite, &eax, t, l);
    }
    ...
    const search = LocalSearch{
        ...
        .max_lk_depth = max_lk_depth,
        .lk_backtrack_limit = options.search.lk_backtrack_limit,
        .lk_backtrack_depth = options.search.lk_backtrack_depth orelse 2,
        .lk_nonseq_branch_limit = options.search.lk_nonseq_branch_limit,
    };

    // EAX-merge every ordered pair to a fixpoint. eaxMergeTours mutates its
    // tour_a parent in place (so the population copy is staged into eax.tour_a
    // first) and reports which parent the improving AB-cycles favored. Improved,
    // re-optimized products re-enter the pool (replace-worst, dedup by edge set),
    // so a later round can recombine a product against a fresh parent — the
    // compounding that a single-trajectory pool never gets.
    const max_rounds: usize = 16;
    var round: usize = 0;
    var improved = true;
    while (improved and round < max_rounds) : (round += 1) {
        improved = false;
        const member_count = elite.count;
        for (0..member_count) |i| {
            for (0..member_count) |j| {
                if (i == j) continue;
                @memcpy(eax.tour_a, elite.tours[i]);
                const outcome = eaxMergeTours(&oracle, &candidates, eax.tour_a, elite.lens[i], elite.tours[j], elite.lens[j], true, &eax);
                stats.eax_merge_attempts += 1;
                if (outcome.cycles_applied == 0) continue;
                stats.eax_merge_cycles += outcome.cycles_applied;
                if (!outcome.winner_is_a) @memcpy(eax.tour_a, eax.tour_b);
                const merged = try reoptimizeRecombinationProduct(search, eax.tour_a, eax.boundary[0..outcome.boundary_count], &stats, &oracle);
                elitePoolOffer(&elite, &eax, eax.tour_a, merged);
                if (merged < best_len) {
                    stats.eax_merge_wins += 1;
                    best_len = merged;
                    @memcpy(best_tour, eax.tour_a);
                    improved = true;
                }
            }
        }
