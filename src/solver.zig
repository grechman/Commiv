...
// Canonical result + telemetry now live in result.zig (a dependency-free module)
// so exact.zig and asymmetric.zig can return the same type without a circular
// import. Re-exported here so every internal `SolveStats` / `SolveResult`
// reference (and `solver.SolveResult` from other modules) keeps resolving.
pub const SolveStats = result_mod.SolveStats;
pub const SolveResult = result_mod.SolveResult;
