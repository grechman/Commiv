...
pub const LocalSearch = struct {
    ...
    // Size band in which the non-sequential Gain23 bridge fallback fires (the
    // O(n)-per-call full scan). Defaults reproduce the historical 256<=n<512
    // gate exactly; raising nonseq_max lets the bridge run at large n (the
    ...
    pub fn improveLK(self: *LocalSearch, stats: *SolveStats, activate_all: bool, full: bool) !u64 {
        ...
        for (0..self.max_passes) |_| {
            ...
            if (self.tour.len >= self.nonseq_min_dimension and self.tour.len < self.nonseq_max_dimension and self.improveGain23Bridge(stats)) {
                moves += 1;
                stats.lk_moves += 1;
                continue;
            }
